...
/* note, this is a modified version by CCP to use the non-blocking
 * stackless mechanism in slsocket.cpp
 */
/*
...
#include "slsocket.h" /* CCP extensions */
#ifndef NO_CARBONIO
#include "CarbonIO/api.h" /* CCP extensions */
#endif
...
#ifdef SLSOCKET
#undef SOCKETCLOSE
#define SOCKETCLOSE(S,R) socket_close((S), (R))
static void sock_linksocket(PySocketSockObject *s);
static void sock_unlinksocket(PySocketSockObject *s);
#ifndef NO_CARBONIO
int g_useSocketIOCP = 0;
#else
#define g_useSocketIOCP 0 /* make this a compile time constant */
#endif
#endif
...
/* Extra error info, for the next exception */
static PyObject *extra_error_info = 0;
void socket_set_extra_error_info(PyObject *o)
{
    PyObject *tmp = extra_error_info;
    extra_error_info = o;
    Py_XINCREF(o);
    Py_XDECREF(tmp);
}
...
set_error(void)
{
    ...
    if (err_no) {
        if (extra_error_info) {
            ...
            PyErr_SetExcFromWindowsErr(socket_error, err_no);
        return NULL;
    }
#endif
...
#ifdef SLSOCKET
int socket_close(SOCKET_T s, void *xtradata)
{
    // dispatch to the runtime-selected implementation
    if ( g_useSocketIOCP == 0 )
    {
        if (xtradata == NULL) {
            /* closing a socket created from a FD */
#ifdef _WIN32
            ...
            return slsock_closesocket( s, xtradata );
    }
#ifndef NO_CARBONIO
    else
    {
        return cio_closehandle( s );
    }
#endif
}


PyObject *socket_set_error(int error)
{
#ifdef MS_WINDOWS
...
#else
    if (extra_error_info) {
        v = Py_BuildValue("(isO)", h_error, "host not found", extra_error_info);
        Py_CLEAR(extra_error_info);
    } else
...
#else
    if (extra_error_info) {
        v = Py_BuildValue("(isO)", error, "getaddrinfo failed", extra_error_info);
        Py_CLEAR(extra_error_info);
    } else
...
static void
init_slsockobject(PySocketSockObject *s,
...
#ifdef SLSOCKET
    else if (!g_useSocketIOCP)
        /* stacklessio sockets are always nonblocking */
        internal_setblocking(s, 0);
#endif
                ...
                )
{
    ...
    if (s != NULL)
        init_slsockobject(s, fd, family, type, proto);
    if (force_timeout != NULL) {
        /* set a specific timeout for the socket.
         * init_sockobject already set blocking to false if defaulttimeout >= 0.
         * but its internal blocking state is otherwise undetermined because
         * it may have been created by an accept() or dup() call, and the inheritance of
         * the blocking attribute for accept is platform-dependent
         */
        int blocking = *force_timeout >= 0.0;
        s->sock_timeout = *force_timeout;
        if (!blocking && defaulttimeout >= 0) {
            /* do nothing, already done by init_sockobject() */
            ;
        } else {
#ifdef SLSOCKET
            if (g_useSocketIOCP) /* stacklessio always uses nonblocking, it has been set already */
#endif
            internal_setblocking(s, blocking);
        }
    }
#ifdef SLSOCKET
    if (!g_useSocketIOCP) {
#ifndef NO_DUP
        if (xtradata == 0) {
            if (slsock_socket_from_fd(&xtradata, fd) < 0) {
                s->sock_fd = -1;
                Py_DECREF(s);
                return NULL;
            }
...
#else
            assert(xtradata != NULL);
#endif
        s->sock_xtradata = xtradata;
    }
    sock_linksocket(s);
#endif
    return s;
...
#if defined(SLSOCKET)
        if (g_useSocketIOCP == 0)
            error = slsock_getaddrinfo(NULL, "0", &hints, &res);
#ifndef NO_CARBONIO
        else
            error = cio_getaddrinfo( NULL, "0", &hints, &res );
#endif
        if (error == -1)
            return -1; /* python exception set by function */
#else
        Py_BEGIN_ALLOW_THREADS
...
#if defined(SLSOCKET)
    if (g_useSocketIOCP == 0) {
        if ((error = slsock_getaddrinfo(name, NULL, &hints, &res)) == -1)
            return -1;
    }
#ifndef NO_CARBONIO
    else {
        error = cio_getaddrinfo( name, NULL, &hints, &res );
    }
#endif
#else
    Py_BEGIN_ALLOW_THREADS
...
sock_accept(PySocketSockObject *s)
{
    ...
    if (g_useSocketIOCP == 0) {
        (void)timeout;
        newfd = slsock_accept(&xtradata, s, SAS2SA(&addrbuf), &addrlen);
        if (newfd == INVALID_SOCKET-1)
            return NULL; /* python exception set */
    }
#ifndef NO_CARBONIO
    else
    {
        newfd = cio_accept( s->sock_fd, SAS2SA(&addrbuf), &addrlen, &timeout, (float)s->sock_timeout );
        if ( timeout == 1 )
        {
    ...
    /* the timeout for the new socket */
    sock_timeout = defaulttimeout;
    if (sock_timeout == -1)
        sock_timeout = s->sock_timeout; /* inherit */
...
#ifdef SLSOCKET
    if (g_useSocketIOCP) /* stacklessio does not modify the internal blocking attribute */
#endif
...
#ifdef SLSOCKET
    if (g_useSocketIOCP) /* stacklessio does not modify the blocking attribute */
#endif
...
#ifndef NO_CARBONIO
//------------------------------------------------------------------------------
static PyObject* enableSSL( PySocketSockObject *sock )
{
    cio_enableSSL( sock->sock_fd );
    Py_RETURN_NONE;
}

//------------------------------------------------------------------------------
static PyObject* setCompressionThreshold( PySocketSockObject *sock, PyObject *args )
{
    return cio_set_global_compression_threshold( sock->sock_fd, args );
}

//------------------------------------------------------------------------------
static PyObject* sock_issocketvalid( PySocketSockObject *sock )
{
    if ( cio_issocket_valid( sock->sock_fd ) )
        Py_RETURN_TRUE;
    else
        Py_RETURN_FALSE;
}
...
sock_connect(PySocketSockObject *s, PyObject *addro)
{
	...
	if (g_useSocketIOCP == 0) {
		timeout = 0;
        res = slsock_connect(s, &addrbuf, addrlen, 1);
	}
#ifndef NO_CARBONIO
    else
        res = cio_connect( s->sock_fd, &addrbuf, addrlen, &timeout, (float)s->sock_timeout );
#endif
    if (res == -1)
        return NULL;
#else
...
sock_connect_ex(PySocketSockObject *s, PyObject *addro)
{
	...
	if (g_useSocketIOCP == 0) {
		timeout = 0;
        res = slsock_connect(s, &addrbuf, addrlen, 0);
	}
#ifndef NO_CARBONIO
    else
        res = cio_connect(s->sock_fd, &addrbuf, addrlen, &timeout, (float)s->sock_timeout );
...
sock_recv(PySocketSockObject *s, PyObject *args)
{
...
#ifdef SLSOCKET
#ifndef NO_CARBONIO
    if (g_useSocketIOCP == 1)
    {
        return cio_recv( s->sock_fd, recvlen, flags, (float)s->sock_timeout );
    }
#endif
#endif
    ...
    {
        Py_buffer bufobj;
        if (PyObject_GetBuffer(buf, &bufobj, PyBUF_SIMPLE))
            return NULL;
        outlen = slsock_recvfrom(s, &bufobj, recvlen, flags, NULL, NULL);
...
#ifndef NO_CARBONIO
    if (g_useSocketIOCP == 1)
        readlen = cio_recv_into( s->sock_fd, buf.buf, recvlen, flags );
    else
#endif
        readlen = slsock_recvfrom(s, &buf, recvlen, flags, NULL, NULL);
#else
    /* Call the guts */
...
sock_recvfrom(PySocketSockObject *s, PyObject *args)
{
    ...
    if (g_useSocketIOCP == 1) {
        ...
        int addrlen;
        buf = cio_recvFrom( s->sock_fd, recvlen, &addrbuf, &addrlen );
        if (!buf)
            return NULL;
        if (!(addr = makesockaddr(s->sock_fd, SAS2SA(&addrbuf), addrlen, s->sock_proto)))
        {
            Py_XDECREF(buf);
            return NULL;
        }
...
#ifdef SLSOCKET
    {
        ...
        outlen = slsock_recvfrom(s, &bufobj, recvlen, flags, &addrbuf, &addrlen);
        PyBuffer_Release(&bufobj);
        if (outlen < 0)
            goto finally;
        if (!(addr = makesockaddr(s->sock_fd, SAS2SA(&addrbuf), addrlen, s->sock_proto)))
...
#ifndef NO_CARBONIO
    if (g_useSocketIOCP == 1)
        readlen = cio_recvFromInto( s->sock_fd, buf.buf, recvlen, &addrbuf, &addrlen );
    else
#endif
        readlen = slsock_recvfrom(s, &buf, recvlen, flags, &addrbuf, &addrlen);
    if (readlen < 0)
        goto error;
    if (!(addr = makesockaddr(s->sock_fd, SAS2SA(&addrbuf), addrlen, s->sock_proto)))
        goto error;
...
#else
    readlen = sock_recvfrom_guts(s, dst, buf, recvlen, flags, &addr);
#endif
    if (readlen < 0) {
        /* Return an error */
        goto error;
...
sock_send(PySocketSockObject *s, PyObject *args)
{
    ...
    if (g_useSocketIOCP == 0) {
        PyObject *obj = 0;
        timeout = 0;
        if (!slsock_parse_send(args, "s*|i:send", "O|i:send", &obj, &pbuf, &flags, 0))
            return NULL;
        timeout = 0;
        n = slsock_sendto(s, obj, &pbuf, flags, NULL, 0);
        if (!obj)
            PyBuffer_Release(&pbuf);
        if (n == -2)
            return 0; //python exception
    }
#ifndef NO_CARBONIO
    else {
        PyObject* obj;
        ...
        {
            n = cio_send_sequence( s->sock_fd, obj, flags );
        } else
            n = -1;
    }
#endif
...
{
    char *buf;
    int len, n = -1, flags = 0, saved_errno;
    Py_buffer pbuf;
    ...
    buf = 0;
    saved_errno = 0;
    if (!slsock_parse_send(args, "s*|i:sendall", "O|i:sendall", &obj, &pbuf, &flags, 0))
        return NULL;
    if (g_useSocketIOCP == 0)   {
        n = slsock_sendall(s, obj, &pbuf, flags);
        if (obj == NULL)
            PyBuffer_Release(&pbuf);
        if (n == -2)
            return 0; /* special python error rhandling */
    }
#ifndef NO_CARBONIO
...
sock_sendto(PySocketSockObject *s, PyObject *args)
{
    ...
    if (g_useSocketIOCP == 0) {
        flags = timeout = 0;
        if (!slsock_parse_send(args, "s*O:sendto", "OO:sendto", &obj, &pbuf, &addro, 0)) {
            PyErr_Clear();
            if (!slsock_parse_send(args, "s*iO:sendto", "OiO:sendto", &obj, &pbuf, &flags, &addro))
                return NULL;
        }
        if (!getsockaddrarg(s, addro, SAS2SA(&addrbuf), &addrlen))
            return NULL;
		timeout = 0;
        n = slsock_sendto(s, obj, &pbuf, flags, &addrbuf, addrlen);
        if (!obj)
            PyBuffer_Release(&pbuf);
        if (n==-2) /* special code to indicate python error */
            return NULL;
    }
#ifndef NO_CARBONIO
    else {
        ...
        flags = 0;
        if ( PyArg_ParseTuple(args, "s*O:sento", &pbuf, &addro)
             || PyArg_ParseTuple(args, "s*iO:sento", &pbuf, &flags, &addro) )
        {
            ...
            PyBuffer_Release(&pbuf);
                   ...
                   || PyArg_ParseTuple(args, "OiO:sendto", &obj, &flags, &addro))
                  && PySequence_Check(obj) )
        {
            PyErr_Clear(); // if one of the parses failed but the other succeeded, undo the fail flag
...
#ifdef SLSOCKET
/* s.sendpacket(data) method */

static PyObject* sock_byteswaiting( PySocketSockObject *sock )
{
#ifndef NO_CARBONIO
    if ( g_useSocketIOCP == 1 )
    {
        return cio_byteswaiting( sock->sock_fd );
    }
#endif
...
static PyObject *
sock_sendpacket(PySocketSockObject *s, PyObject *args)
{
    if (g_useSocketIOCP == 0) {
        Py_buffer pbuf;
        int n;
        ...
        n = slsock_sendpacket(s, obj, &pbuf);
        PyBuffer_Release(&pbuf);
        if (n<0)
            return 0;
    }
        ...
        if (!cio_sendpacket_ex( s->sock_fd, args ))
            return 0;
#endif
...
static PyObject *
sock_recvpacket(PySocketSockObject *s, PyObject *args)
{
    if (!PyArg_ParseTuple(args, ":recvpacket"))
        return 0;

    if (g_useSocketIOCP == 0)
         return slsock_recvpacket(s);
#ifndef NO_CARBONIO
    else
        return cio_recvpacket_ex(s->sock_fd);
#endif
}
PyDoc_STRVAR(recvpacket_doc,
"recvpacket()\n\
\n\
Reseve a single packet from the socket, which has been sent with\n\
sendpacket.\n\
This calls recv() repeatedly until all data is received.");

static PyObject *sock_recvpacketoob( PySocketSockObject *s, PyObject *args )
{
    if (!PyArg_ParseTuple(args, ":recvpacketoob"))
        return 0;
    if (g_useSocketIOCP == 0)
        return slsock_recvpacket_oob(s);
#ifndef NO_CARBONIO
    else
        return cio_recvpacketoob_ex(s->sock_fd);
#endif
}
...
static PyObject *
sock_setblockingsend(PySocketSockObject *s, PyObject *args)
{
    PyObject *o = 0, *result;
...
#ifndef NO_CARBONIO
    if (g_useSocketIOCP == 1) {
        oldvalue = cio_setblockingsend(s->sock_fd, newvalue);
        if (!o)
            cio_setblockingsend(s->sock_fd, oldvalue);
    }
    else
#endif
    {
        oldvalue = slsock_setblockingsend(s, newvalue);
        if (!o)
            slsock_setblockingsend(s, oldvalue);
    }
    result = oldvalue ? Py_True : Py_False;
    Py_INCREF(result);
    return result;
}
...
static PyObject *
sock_setzerobytereads(PySocketSockObject *s, PyObject *args)
{
    if (g_useSocketIOCP == 0) {
        PyObject *o = 0, *result;
        int newvalue, oldvalue;
        if (!PyArg_ParseTuple(args, "|O:setzerobytereads", &o))
            return 0;
        newvalue = o ? PyObject_IsTrue(o) : 0;
        oldvalue = slsock_setzerobytereads(s, newvalue);
        if (!o)
            slsock_setzerobytereads(s, oldvalue);  /*if we are querying only */
        result = oldvalue ? Py_True : Py_False;
        Py_INCREF(result);
        return result;
    } else {
        // IOCP dynamically detects the need for zero-byte reads,
        // always 'succeed' when called
        Py_RETURN_TRUE;
    }
}
...
static PyObject *
sock_setmaxpacketsize(PySocketSockObject *s, PyObject *args)
{
    if (g_useSocketIOCP == 0) {
...
static PyObject *
sock_getstats(PySocketSockObject *s)
{
    if (g_useSocketIOCP == 0)
        return slsock_getstats(s);
#ifndef NO_CARBONIO
    else
        return cio_getstats(s->sock_fd);
#endif
}
...
static PyObject *
socket_getglobalstats(PyObject *m)
{
    if (g_useSocketIOCP == 0)
        return slsock_getstats(0);
#ifndef NO_CARBONIO
    else
        return cio_getstats(0);
#endif
}

PyDoc_STRVAR(getglobalstats_doc,
"getstats()\n\
\n\
Return usage statsistics for the socket module");

static PySocketSockObject *allSockets = 0;
static void sock_linksocket(PySocketSockObject *s) {
    if (allSockets) {
        s->sock_next = allSockets;
        s->sock_prev = allSockets->sock_prev;
        s->sock_next->sock_prev = s;
        s->sock_prev->sock_next = s;
    } else {
        s->sock_prev = s->sock_next = allSockets = s;
    }
}
static void sock_unlinksocket(PySocketSockObject *s) {
    if (s->sock_next == 0)
        return;
    if (s->sock_next != s) {
        s->sock_next->sock_prev = s->sock_prev;
        s->sock_prev->sock_next = s->sock_next;
        if (allSockets == s)
            allSockets = s->sock_next;
    } else {
        allSockets = 0;
    }
}
...
static PyObject *
socket_getsockets(PyObject *m)
{
    if (!allSockets)
        Py_RETURN_NONE;
    Py_INCREF(allSockets);
    return (PyObject*)allSockets;
}

PyDoc_STRVAR(getsockets_doc,
"getsockets()\n\
\n\
Return the first in a linked list of active sockets");

#ifndef NO_CARBONIO
static PyObject *
socket_use_carbonio(PyObject *m, PyObject *O)
{
    long old, v;
    old = g_useSocketIOCP;
    if (O != Py_None) {
...
PyDoc_STRVAR(socket_use_carbonio_doc,
"use_carbonio()\n\
When called with True, CarbonIO will be used for socket communications, \
else Stacklessio.\n\
Call with None to query.");
#endif
...
sock_shutdown(PySocketSockObject *s, PyObject *arg)
{
    ...
    if ( g_useSocketIOCP == 1 )
    {
        res = cio_shutdown( s->sock_fd, how );
    }
    else
#endif
#endif
...
#endif
#ifdef SLSOCKET
    {"byteswaiting", (PyCFunction)sock_byteswaiting, METH_NOARGS, 0},
    {"sendpacket", (PyCFunction)sock_sendpacket, METH_VARARGS, sendpacket_doc},
    {"recvpacket", (PyCFunction)sock_recvpacket, METH_VARARGS, recvpacket_doc},
...
#ifndef NO_CARBONIO
    {"valid", (PyCFunction)sock_issocketvalid, METH_NOARGS, 0 },
    {"setCompressionThreshold", (PyCFunction)setCompressionThreshold, METH_VARARGS, 0 },
    {"enableSSL", (PyCFunction)enableSSL, METH_NOARGS, 0 },
#endif
    {"getSocketDescriptor", (PyCFunction)getSocketDescriptor, METH_NOARGS, 0 },
#endif
    {NULL,                      NULL}           /* sentinel */
};
...
#ifdef SLSOCKET
       {"next", T_OBJECT, offsetof(PySocketSockObject, sock_next), READONLY, "next socket"},
       {"prev", T_OBJECT, offsetof(PySocketSockObject, sock_next), READONLY, "previous socket"},
#endif
       {0},
};
...
sock_dealloc(PySocketSockObject *s)
{
    ...
    if (s->sock_fd != -1)
#ifdef SLSOCKET
        (void) SOCKETCLOSE(s->sock_fd, s->sock_xtradata);
    if (g_useSocketIOCP == 0)
        slsock_releasesocket(s);
#else
...
sock_repr(PySocketSockObject *s)
{
    ...
    char *fmt;
#if SIZEOF_SOCKET_T > SIZEOF_LONG
    fmt = "<socket object, fd=%Id, family=%d, type=%d, protocol=%d>";
#else
...
#ifdef SLSOCKET
        ((PySocketSockObject *)new)->sock_next = 0;
#endif
    }
    return new;
}
...
#ifdef SLSOCKET
    if (g_useSocketIOCP == 0) {
        fd = slsock_socket(&xtradata, family, type, proto);
        if (fd == -2)
            return -1; /* special python exception code */
    }
#ifndef NO_CARBONIO
    else
        fd = cio_socket( family, type, proto );
#endif
#else
    Py_BEGIN_ALLOW_THREADS
...
{
#if defined HAVE_GETHOSTNAME && !HAVE_GETHOSTNAME //CCP change, PS3
    PyErr_SetString(PyExc_NotImplementedError, "gethosname");
    return 0;
#else
    char buf[1024];
    int res;
...
#ifdef SLSOCKET
    if (g_useSocketIOCP == 0)
        h = slsock_gethostbyname(name);
#ifndef NO_CARBONIO
    else
        h = (struct hostent*)cio_gethostbyname(name);
#endif
    if (h == (struct hostent*)-1)
        return NULL;
#else
    Py_BEGIN_ALLOW_THREADS
#ifdef HAVE_GETHOSTBYNAME_R
...
#ifdef SLSOCKET
    if (g_useSocketIOCP == 0)
        h = slsock_gethostbyaddr(ap, al, af);
#ifndef NO_CARBONIO
    else
        h = (struct hostent*)cio_gethostbyaddr(ap, al, af);
#endif
    if (h == (struct hostent *)-1)
        return NULL;
#else
...
socket_getservbyname(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "s|s:getservbyname", &name, &proto))
        return NULL;
#if !defined HAVE_GETSERVBYNAME || HAVE_GETSERVBYNAME //CCP change, PS3
    Py_BEGIN_ALLOW_THREADS
...
#if !defined HAVE_GETSERVBYPORT || HAVE_GETSERVBYPORT //CCP change, PS3
    Py_BEGIN_ALLOW_THREADS
...
socket_getprotobyname(PyObject *self, PyObject *args)
{
    char *name;
    struct protoent *sp;
#if defined __BEOS__ || (defined HAVE_GETPROTOBYNAME && !HAVE_GETPROTOBYNAME) //CCP change, PS3
...
socket_socketpair(PyObject *self, PyObject *args)
{
    ...
    if (res == NULL) {
        if (s0 == NULL)
#ifdef SLSOCKET
            SOCKETCLOSE(sv[0], NULL);
#else
            SOCKETCLOSE(sv[0]);
#endif
        if (s1 == NULL)
#ifdef SLSOCKET
            SOCKETCLOSE(sv[1], NULL);
#else
            SOCKETCLOSE(sv[1]);
#endif
    }
...
socket_fromfd(PyObject *self, PyObject *args)
{
    ...
    if (fd < 0)
        return set_error();
    s = new_sockobject(fd, family, type, proto, NULL
#ifdef SLSOCKET
                       , NULL
#endif
                       );
...
#ifdef SLSOCKET
    if (g_useSocketIOCP == 0) {
        error = slsock_getaddrinfo(hptr, pptr, &hints, &res0);
        if (error == -1)
            goto err;
    }
#ifndef NO_CARBONIO
    else {
        error = cio_getaddrinfo( hptr, pptr, &hints, &res0 );
    }
#endif
#else
    Py_BEGIN_ALLOW_THREADS
...
#ifdef SLSOCKET
    if (g_useSocketIOCP == 0) {
        error = slsock_getaddrinfo(hostp, pbuf, &hints, &res);
        if (error == -1)
            goto fail;
    }
#ifndef NO_CARBONIO
...
#ifdef SLSOCKET
    {"getstats",                (PyCFunction) socket_getglobalstats,
    METH_NOARGS, getglobalstats_doc},
    {"getsockets",              (PyCFunction) socket_getsockets,
    METH_NOARGS, getsockets_doc},
    {"get_settings",            (PyCFunction) slsock_get_settings,
    METH_NOARGS, ""},
    {"apply_settings",          (PyCFunction) slsock_apply_settings,
    METH_O, ""},
#ifndef NO_CARBONIO
    {"use_carbonio",            (PyCFunction) socket_use_carbonio,
    METH_O, socket_use_carbonio_doc},
#endif
#endif
    {NULL,                      NULL}            /* Sentinel */
};
...
#if defined(SOCK_SEQPACKET) //CCP change, for PS3

...
#ifdef SLSOCKET
#undef SOCKETCLOSE
#define SOCKETCLOSE(S,R) slsocket_close((S), (R))
...
set_error(void)
{
    ...
    if (err_no)
	{
		if( extra_error_info )
		{
			...
			PyErr_SetExcFromWindowsErrWithFilename( PyExc_OSError, err_no, text ? text : "" );
...
#ifdef SLSOCKET
int slsocket_close(SOCKET_T s, void *xtradata)
{
                ...
                )
{
    ...
    if (s == NULL)
        return NULL;
    if (init_slsockobject(s, fd, family, type, proto) == -1) {
        Py_DECREF(s);
        return NULL;
    }
#ifdef SLSOCKET
...
#ifndef NO_CARBONIO
    if (g_useSocketIOCP == 1)
        readlen = cio_recv_into( s->sock_fd, buf, recvlen, flags );
    else
#endif
        readlen = slsock_recvfrom(s, &pbuf, recvlen, flags, NULL, NULL);
#else
    /* Call the guts */
...
sock_recvfrom(PySocketSockObject *s, PyObject *args)
{
    ...
    if (g_useSocketIOCP == 1) {
        sock_addr_t addrbuf;
        int addrlen;
        buf = cio_recvFrom( s->sock_fd, recvlen, SAS2SA(&addrbuf), &addrlen );
        if (!buf)
            return NULL;
...
#ifdef SLSOCKET
    {
        ...
        outlen = slsock_recvfrom(s, &bufobj, recvlen, flags, SAS2SA(&addrbuf), &addrlen);
        PyBuffer_Release(&bufobj);
        if (outlen < 0)
            goto finally;
        if (!(addr = makesockaddr(s->sock_fd, SAS2SA(&addrbuf), addrlen, s->sock_proto)))
...
#ifndef NO_CARBONIO
    if (g_useSocketIOCP == 1)
        readlen = cio_recvFromInto( s->sock_fd, pbuf.buf, recvlen, SAS2SA(&addrbuf), &addrlen );
    else
#endif
        readlen = slsock_recvfrom(s, &pbuf, recvlen, flags, SAS2SA(&addrbuf), &addrlen);
    if (readlen < 0)
        goto error;
    if (!(addr = makesockaddr(s->sock_fd, SAS2SA(&addrbuf), addrlen, s->sock_proto)))
        goto error;
...
sock_send(PySocketSockObject *s, PyObject *args)
{
    ...
    if (g_useSocketIOCP == 0) {
		...
		ctx.len = pbuf.len;
		ctx.flags = flags;
        ctx.result = slsock_sendto(s, obj, &pbuf, flags, NULL, 0);
        if (!obj)
            PyBuffer_Release(&pbuf);
        if (ctx.result == -2)
        ...
        if (PyArg_ParseTuple(args, "s*|i:send", &pbuf, &flags)) {
            ...
            ctx.result = cio_send( s->sock_fd, pbuf.buf, (int)pbuf.len, flags );
            PyBuffer_Release(&pbuf);
        } else if (PyArg_ParseTuple(args, "O|i:send", &obj, &flags)
                   && PySequence_Check(obj) )
        {
            ctx.result = cio_send_sequence( s->sock_fd, obj, flags );
        } else
            ctx.result = -1;
    }
#endif
...
sock_sendto(PySocketSockObject *s, PyObject *args)
{
    ...
    if (g_useSocketIOCP == 0) {
        ...
        flags = 0;
        if ( PyArg_ParseTuple(args, "y*O:sento", &pbuf, &addro)
             || PyArg_ParseTuple(args, "y*iO:sento", &pbuf, &flags, &addro) )
        {
